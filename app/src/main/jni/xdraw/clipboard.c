...
void copy(const char *text, long seconds_active,bool isPrimary) {
    ...
    //XSetSelectionOwner(display, SELECTION2, window, 0);
    if (XGetSelectionOwner(display, SELECTION) != window)
        return;

    bool isRecive = false;
    time_t end = time(NULL) +
                 seconds_active; //declare end time equal to current time + seconds_active seconds
        ...
        switch (event.type) {
            ...
            case SelectionRequest:
                ...
                if (xse.target == atom_targets) {
                  //  isRecive = true;
                    r = XChangeProperty(xse.display, xse.requestor, xse.property, XA_ATOM, 32,
                                        PropModeReplace, (unsigned char *) &UTF8, 1);
                    XFlush(mdisplay);
                } else if (xse.target == XA_STRING || xse.target == ATOM_TEXT) {
                  //  isRecive = true;
                    r = XChangeProperty(xse.display, xse.requestor, xse.property, XA_STRING, 8,
                                        PropModeReplace, (unsigned char *) text, size);
                    XFlush(mdisplay);
                } else if (xse.target == UTF8) {
                   // isRecive = true;
                    r = XChangeProperty(xse.display, xse.requestor, xse.property, UTF8, 8,
                                        PropModeReplace, (unsigned char *) text, size);
                    XFlush(mdisplay);
                } else
                    xse.property = None;
        ...
        case SelectionNotify:
            ...
            if (event.xselection.property) {
                ...
                XGetWindowProperty(event.xselection.display, event.xselection.requestor,
                                   ...
                                   &atom_targets, &format, &size, &n, (unsigned char **) &data);
